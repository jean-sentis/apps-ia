    ...
    $(document).ready(function() {
        ...
        function initCodePostalCommune(cpId, communeId) {
            ...
            function fetchCommunes() {
                ...
                $.getJSON('https://geo.api.gouv.fr/communes?codePostal=' + cp + '&fields=nom')
                    .done(function(communes) {
                        ...
                        if (communes && communes.length) {
                            ...
                            $commune.append($('<option>').val('').text('Aucune commune trouvée'));
                        }
                    })
                    .fail(function() {
                        $commune.find('option:gt(0)').remove();
            ...
            $photosInput.on('change', function() {
                ...
                for (var i = 0; i < files.length; i++) {
                    ...
                    reader.onload = (function(idx) {
                        return function(e) {
                            var $img = $('<img>').attr('src', e.target.result).addClass('lmd-vignette-img');
        ...
        $('#lmd-estimation-form').on('submit', function(e) {
            ...
            $.ajax({
                ...
                $msg.show().css('color', r.success ? 'green' : 'red').text(r.data && r.data.message ? r.data.message : (r.success ? 'Envoyé !' : 'Erreur'));
                if (r.success) {
                    $form[0].reset();
                }
            }).fail(function() {
                $msg.show().css('color', 'red').text('Erreur réseau. Réessayez.');
            });
        });
    });
})(jQuery);

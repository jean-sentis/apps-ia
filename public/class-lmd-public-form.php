...
class LMD_Public_Form {
    ...
    public function render($atts = []) {
        ...
        ?>
        <div class="lmd-form-wrapper" data-style="<?php echo esc_attr($style); ?>">
            ...
            <div class="lmd-right-section">
                <form id="lmd-estimation-form" enctype="multipart/form-data">
                    ...
                    <p><label>Téléphone <span class="lmd-optional">(facultatif)</span> <input type="text" name="telephone" /></label></p>
                    <p><label>Description <textarea name="description" rows="4"></textarea></label></p>
                    <p><button type="submit" class="button">Envoyer</button></p>
                </form>
                <div id="lmd-form-message" style="display:none;"></div>
            </div>
        </div>
        <?php
    }
    ...
    public function handle_submission() {
        ...
        if (isset($_POST['lmd_nonce']) && !wp_verify_nonce($_POST['lmd_nonce'], 'lmd_public')) {
            if (function_exists('ob_end_clean')) {
                ob_end_clean();
            }
            wp_send_json_error(['message' => 'Nonce invalide']);
        }
        $email = isset($_POST['email']) ? sanitize_email(wp_unslash($_POST['email'])) : (isset($_POST['mail']) ? sanitize_email(wp_unslash($_POST['mail'])) : '');
        if (empty($email)) {
            if (function_exists('ob_end_clean')) {
                ob_end_clean();
            }
        ...
        $nom = isset($_POST['nom']) ? sanitize_text_field(wp_unslash($_POST['nom'])) : (isset($_POST['name']) ? sanitize_text_field(wp_unslash($_POST['name'])) : '');
        if (empty($nom)) {
            if (function_exists('ob_end_clean')) {
                ob_end_clean();
            }
        ...
        $civility = isset($_POST['civility']) ? sanitize_text_field(wp_unslash($_POST['civility'])) : '';
        ...
        $phone = isset($_POST['telephone']) ? sanitize_text_field(wp_unslash($_POST['telephone'])) : (isset($_POST['phone']) ? sanitize_text_field(wp_unslash($_POST['phone'])) : '');

...
class LMD_Admin {
    ...
    public function __construct() {
        ...
        add_action('admin_post_lmd_reset_analysis_for_test', [$this, 'handle_reset_analysis_for_test']);
        add_action('admin_post_lmd_export_consumption', [$this, 'handle_export_consumption']);
        add_action('admin_post_lmd_export_product_margin', [$this, 'handle_export_product_margin']);
        add_action('admin_post_lmd_save_margin_fx', [$this, 'handle_save_margin_fx']);
        add_action('admin_post_lmd_sandbox_seed', [$this, 'handle_sandbox_seed']);
        add_action('admin_post_lmd_sandbox_clear', [$this, 'handle_sandbox_clear']);
        add_action('admin_post_lmd_export_activity', [$this, 'handle_export_activity']);
        add_action('admin_post_lmd_export_full_copy', [$this, 'handle_export_full_copy']);
        add_action('admin_post_lmd_import_full_copy', [$this, 'handle_import_full_copy']);
    }
    ...
    public function maybe_redirect_legacy_app_pages() {
        if (!is_admin() || !isset($_GET['page']) || isset($_GET['networkwide'])) {
            return;
        }
        $legacy = [
    ...
    public function handle_delete_estimation() {
        ...
        if (!$id || !wp_verify_nonce($_GET['_wpnonce'] ?? '', 'lmd_delete_estimation_' . $id)) {
            wp_die('Non autorisé');
        }
        ...
        global $wpdb;
        $wpdb->delete($wpdb->prefix . 'lmd_estimation_tags', ['estimation_id' => $id], ['%d']);
    ...
    public function handle_bulk_delete_estimations() {
        if (!current_user_can('manage_options') || !wp_verify_nonce($_POST['_wpnonce'] ?? '', 'lmd_bulk_delete')) {
            wp_die('Non autorisé');
        }
    ...
    public function handle_reset_analysis_for_test() {
        if (!current_user_can('manage_options') || !wp_verify_nonce($_GET['_wpnonce'] ?? '', 'lmd_reset_analysis_test')) {
            wp_die('Non autorisé');
        }
        global $wpdb;
        ...
        foreach (['avis1_titre', 'avis1_dimension', 'avis2_titre', 'avis2_dimension'] as $c) {
        ...
        $ids = $wpdb->get_col($wpdb->prepare("SELECT id FROM $table WHERE site_id = %d", $site_id));
        if (!empty($ids)) {
            $placeholders = implode(',', array_fill(0, count($ids), '%d'));
            $wpdb->query($wpdb->prepare("DELETE FROM $et_table WHERE estimation_id IN ($placeholders)", $ids));
        }

        $count = count($ids);
        wp_redirect(add_query_arg('lmd_reset_done', $count, admin_url('admin.php?page=lmd-app-estimation&tab=help')));
        exit;
    }

    public function handle_export_consumption() {
        if (!current_user_can('manage_options') || !wp_verify_nonce($_GET['_wpnonce'] ?? '', 'lmd_export_consumption')) {
            wp_die('Non autorisé');
        }
        ...
        header('Content-Disposition: attachment; filename="consommation-ia-' . $month . '.csv"');
        $out = fopen('php://output', 'w');
        fprintf($out, chr(0xEF) . chr(0xBB) . chr(0xBF));
        fputcsv($out, ['Client', 'Site ID', 'Analyses', 'SerpAPI (unités)', 'SerpAPI ($)', 'Firecrawl (unités)', 'Firecrawl ($)', 'ImgBB (unités)', 'ImgBB ($)', 'Gemini (unités)', 'Gemini ($)', 'Total ($)'], ';');
        ...
        fputcsv($out, ['TOTAL', '', $agg['analyses_count'], $agg['by_api']['serpapi']['units'], $agg['by_api']['serpapi']['cost_usd'], $agg['by_api']['firecrawl']['units'], $agg['by_api']['firecrawl']['cost_usd'], $agg['by_api']['imgbb']['units'], $agg['by_api']['imgbb']['cost_usd'], $agg['by_api']['gemini']['units'], $agg['by_api']['gemini']['cost_usd'], $agg['total_usd']], ';');
        fclose($out);
        exit;
    }

    public function handle_export_activity() {
        if (!current_user_can('manage_options') || !wp_verify_nonce($_GET['_wpnonce'] ?? '', 'lmd_export_activity')) {
            wp_die('Non autorisé');
        }
        $month = isset($_GET['month']) ? sanitize_text_field($_GET['month']) : date('Y-m');
        if (!preg_match('/^\d{4}-\d{2}$/', $month)) {
        ...
        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename="activite-fonctionnalites-' . $month . '.csv"');
        $out = fopen('php://output', 'w');
        fprintf($out, chr(0xEF) . chr(0xBB) . chr(0xBF));
        ...
        fputcsv($out, ['Client', 'Site ID', 'Signature configurée', 'Utilisateurs avec signature'], ';');
    ...
    public function handle_export_full_copy() {
        if (!current_user_can('manage_options') || !wp_verify_nonce($_GET['_wpnonce'] ?? '', 'lmd_export_full_copy')) {
            wp_die('Non autorisé');
        }
        ...
        try {
            ...
            $zip_path = $exporter->export($site_id ?: null, $include_photos, $include_api_keys);
    ...
    public function handle_import_full_copy() {
        if (!current_user_can('manage_options') || !wp_verify_nonce($_POST['_wpnonce'] ?? '', 'lmd_import_full_copy')) {
            wp_die('Non autorisé');
        }
        $replace = !empty($_POST['lmd_import_replace']);
        $file = $_FILES['lmd_import_file'] ?? null;
        if (!$file || !empty($file['error']) || empty($file['tmp_name'])) {
            wp_redirect(add_query_arg('lmd_import_error', 'fichier', admin_url('admin.php?page=lmd-copy-export-import')));
            exit;
        }
        try {
            $importer = new LMD_Full_Export_Import();
            $stats = $importer->import($file['tmp_name'], $replace);
            wp_redirect(add_query_arg('lmd_import_ok', 1, admin_url('admin.php?page=lmd-copy-export-import')));
            exit;
        } catch (Exception $e) {
            wp_redirect(add_query_arg('lmd_import_error', urlencode($e->getMessage()), admin_url('admin.php?page=lmd-copy-export-import')));
            exit;
        }
    }

    public function handle_new_estimation() {
        if (!current_user_can('manage_options') || !wp_verify_nonce($_POST['_wpnonce'] ?? '', 'lmd_new_estimation')) {
            wp_die('Non autorisé');
        }
        ...
        $civility = sanitize_text_field(wp_unslash($_POST['client_civility'] ?? ''));
        $code_postal = sanitize_text_field(wp_unslash($_POST['client_postal_code'] ?? ''));
        ...
        $insert_data = [
            ...
            'client_civility' => in_array($civility, ['Monsieur', 'Madame'], true) ? $civility : null,
            'client_first_name' => sanitize_text_field(wp_unslash($_POST['client_first_name'] ?? '')),
            'client_email' => sanitize_email(wp_unslash($_POST['client_email'] ?? '')),
            'client_phone' => sanitize_text_field(wp_unslash($_POST['client_phone'] ?? '')),
            'client_postal_code' => preg_match('/^[0-9]{5}$/', $code_postal) ? $code_postal : null,
            'client_commune' => sanitize_text_field(wp_unslash($_POST['client_commune'] ?? '')) ?: null,
            'description' => sanitize_textarea_field(wp_unslash($_POST['description'] ?? '')),
            'photos' => $photos_json,
            'status' => 'new',
            'source' => 'admin',
        ];
        $insert_fmt = ['%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s'];
        if (in_array('dimensions', $cols, true)) {
            $insert_data['dimensions'] = $dimensions ?: null;
            $insert_fmt[] = '%s';
        }
        $wpdb->insert($wpdb->prefix . 'lmd_estimations', $insert_data, $insert_fmt);
        wp_redirect(admin_url('admin.php?page=lmd-app-estimation&tab=list'));
        exit;
    }
    ...
    public function add_menu() {
        if (is_network_admin()) {
            ...
            add_submenu_page('lmd-remontee', 'Remontée statistique', 'Remontée', 'manage_network', 'lmd-remontee', [$this, 'render_remontee']);
            return;
        }
        ...
        if ($is_parent) {
            ...
            add_submenu_page('lmd-apps-ia', 'Copie export/import', null, 'manage_options', 'lmd-copy-export-import', [$this, 'render_copy_export_import']);
        }
        if ($is_parent && !is_multisite()) {
            add_submenu_page('lmd-apps-ia', 'Outils bac à sable', null, 'manage_options', 'lmd-sandbox-tools', [$this, 'render_sandbox_tools']);
        }

        // menu_title = null : page enregistrée et accessible (admin.php?page=…) mais pas d’entrée dans le menu latéral (WP 5.3+).
        add_submenu_page('lmd-apps-ia', 'Nouvelle demande', null, 'manage_options', 'lmd-new-estimation', [$this, 'render_new_estimation']);
    ...
    public function remove_parent_only_menu_items() {
        ...
        $parent_only = ['lmd-activity', 'lmd-consumption', 'lmd-product-margin', 'lmd-api-config', 'lmd-promotions', 'lmd-copy-export-import'];
    ...
    public function enqueue_assets($hook) {
        ...
        wp_enqueue_style('lmd-admin', LMD_PLUGIN_URL . 'assets/admin-style.css', [], LMD_VERSION);
        $tab_list = isset($_GET['tab']) && $_GET['tab'] === 'list';
        if (strpos($hook, 'lmd-estimations-list') !== false || ($hook === 'lmd-apps-ia_page_lmd-app-estimation' && $tab_list)) {
            wp_enqueue_script('sortablejs', 'https://cdn.jsdelivr.net/npm/sortablejs@1.15.2/Sortable.min.js', [], '1.15.2', true);
        }
        wp_enqueue_script('lmd-admin', LMD_PLUGIN_URL . 'assets/admin.js', ['jquery'], LMD_VERSION, true);
    ...
    public function handle_save_margin_fx() {
        if (!current_user_can('manage_options') || !wp_verify_nonce($_POST['lmd_margin_fx_nonce'] ?? '', 'lmd_margin_fx')) {
            wp_die('Non autorisé');
        }
        if (is_multisite() && !is_main_site()) {
            wp_die('Non autorisé');
        }
        $fx = isset($_POST['lmd_margin_usd_to_eur']) ? (float) str_replace(',', '.', (string) wp_unslash($_POST['lmd_margin_usd_to_eur'])) : 0.92;
        if ($fx <= 0) {
            $fx = 0.92;
        }
        update_option('lmd_margin_usd_to_eur', $fx);
        $month = isset($_POST['redirect_month']) ? sanitize_text_field(wp_unslash($_POST['redirect_month'])) : gmdate('Y-m');
        if (!preg_match('/^\d{4}-\d{2}$/', $month)) {
            $month = gmdate('Y-m');
        }
        if (!empty($_POST['redirect_lmd_hub'])) {
            wp_safe_redirect(add_query_arg(['page' => 'lmd-apps-ia', 'hub_tab' => 'margin', 'month' => $month, 'updated' => '1'], admin_url('admin.php')));
        } else {
            wp_safe_redirect(add_query_arg(['page' => 'lmd-product-margin', 'month' => $month, 'updated' => '1'], admin_url('admin.php')));
        }
        exit;
    }

    public function handle_export_product_margin() {
        if (!current_user_can('manage_options') || !wp_verify_nonce($_GET['_wpnonce'] ?? '', 'lmd_export_product_margin')) {
            wp_die('Non autorisé');
        }
        if (is_multisite() && !is_main_site()) {
            wp_die('Non autorisé');
        ...
        $all_clients = !is_multisite() || get_current_blog_id() === 1;
        $report = $usage->get_parent_product_margin_report($month, $all_clients);
        $p = $report['products'][0] ?? null;

        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename="marge-produits-' . $month . '.csv"');
        $out = fopen('php://output', 'w');
        fprintf($out, chr(0xEF) . chr(0xBB) . chr(0xBF));
        fputcsv($out, ['Mois', 'Produit', 'Analyses', 'CA HT (EUR)', 'Cout API (USD)', 'Cout API (EUR)', 'Marge (EUR)', 'Marge %', 'CA moy / analyse (EUR)', 'Cout API moy / analyse (EUR)', 'Prix moy payant (EUR)', 'Analyses payantes'], ';');
        if ($p) {
            fputcsv($out, [
                $month,
                $p['label'],
        ...
        fputcsv($out, [], ';');
        fputcsv($out, ['Detail par site', 'Site ID', 'Nom', 'Analyses mois', 'Payantes mois', 'CA HT (EUR)'], ';');
    ...
    public function render_sandbox_tools() {
        if (!class_exists('LMD_Sandbox_Tools') || !LMD_Sandbox_Tools::is_allowed()) {
            wp_die('Cette page est réservée à un WordPress en site unique (bac à sable).');
        }
    ...
    public function handle_sandbox_seed() {
        if (!current_user_can('manage_options') || !wp_verify_nonce($_POST['_wpnonce'] ?? '', 'lmd_sandbox_seed')) {
            wp_die('Non autorisé');
        }
        if (!class_exists('LMD_Sandbox_Tools') || !LMD_Sandbox_Tools::is_allowed()) {
            wp_die('Non autorisé');
        }
        $n = isset($_POST['lmd_sandbox_count']) ? (int) $_POST['lmd_sandbox_count'] : 3;
    ...
    public function handle_sandbox_clear() {
        if (!current_user_can('manage_options') || !wp_verify_nonce($_POST['_wpnonce'] ?? '', 'lmd_sandbox_clear')) {
            wp_die('Non autorisé');
        }
        if (!class_exists('LMD_Sandbox_Tools') || !LMD_Sandbox_Tools::is_allowed()) {
            wp_die('Non autorisé');
        ...
        $err = is_wp_error($r) ? $r->get_error_message() : '';
        wp_safe_redirect(add_query_arg(['page' => 'lmd-sandbox-tools', 'cleared' => $deleted, 'err' => $err ? rawurlencode($err) : ''], admin_url('admin.php')));
        exit;
    }
}

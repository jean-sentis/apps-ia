...
add_action(
    "wp_ajax_lmd_run_analysis_background",
    "lmd_ajax_run_analysis_background",
);
add_action(
    "wp_ajax_nopriv_lmd_run_analysis_background",
    "lmd_ajax_run_analysis_background",
);
...
add_action(
    "wp_ajax_lmd_check_analysis_status",
    "lmd_ajax_check_analysis_status",
);
add_action(
    "wp_ajax_lmd_check_analysis_status_batch",
    "lmd_ajax_check_analysis_status_batch",
);
...
add_action(
    "wp_ajax_lmd_save_analysis_pricing",
    "lmd_ajax_save_analysis_pricing",
);
...
function lmd_ajax_save_lot_number()
{
    ...
    $lot = isset($_POST["lot_number"])
        ? preg_replace("/\D/", "", wp_unslash($_POST["lot_number"]))
        : "";
    if ($lot !== "") {
        $n = (int) $lot;
        if ($n < 1 || $n > 999) {
            ...
            $lot = str_pad((string) $n, 3, "0", STR_PAD_LEFT);
        }
    }
    global $wpdb;
...
function lmd_ajax_launch_analysis()
{
    ...
    if (in_array("first_viewed_at", $cols, true)) {
        $row = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT first_viewed_at FROM $table WHERE id = %d",
                $id,
            ),
        );
    ...
    if (in_array("ai_launch_count", $cols, true)) {
        $wpdb->query(
            $wpdb->prepare(
                "UPDATE $table SET ai_launch_count = COALESCE(ai_launch_count, 0) + 1 WHERE id = %d",
                $id,
            ),
        );
    }
    ...
    set_transient(
        "lmd_analysis_progress_" . $id,
        ["percent" => 5, "step" => "Démarrage..."],
        300,
    );

    @set_time_limit(300);
    $processor = class_exists("LMD_Estimation_Processor")
        ? new LMD_Estimation_Processor()
        : null;
...
function lmd_ajax_save_avis1_estimates()
{
    ...
    global $wpdb;
    $data = [
        "avis1_estimate_low" => isset($_POST["estimate_low"])
            ? floatval(
                str_replace(
                    [" ", ","],
                    ["", "."],
                    (string) $_POST["estimate_low"],
                ),
            )
            : null,
        "avis1_estimate_high" => isset($_POST["estimate_high"])
            ? floatval(
                str_replace(
                    [" ", ","],
                    ["", "."],
                    (string) $_POST["estimate_high"],
                ),
            )
            : null,
        "avis1_prix_reserve" => isset($_POST["prix_reserve"])
            ? floatval(
                str_replace(
                    [" ", ","],
                    ["", "."],
                    (string) $_POST["prix_reserve"],
                ),
            )
            : null,
    ];
    if (isset($_POST["avis_text"])) {
        $data["auctioneer_notes"] = sanitize_textarea_field(
            wp_unslash($_POST["avis_text"]),
        );
    }
    if (isset($_POST["avis_titre"])) {
        $data["avis1_titre"] = sanitize_text_field(
            wp_unslash($_POST["avis_titre"]),
        );
    }
    if (isset($_POST["avis_dimension"])) {
        $data["avis1_dimension"] = sanitize_text_field(
            wp_unslash($_POST["avis_dimension"]),
        );
    }
    $wpdb->update(
        $wpdb->prefix . "lmd_estimations",
    ...
    wp_send_json_success(
        $tag_display ? ["tag_estimation" => $tag_display] : [],
    );
}
...
function lmd_ajax_save_avis2_estimates()
{
    ...
    global $wpdb;
    $data = [
        "avis2_estimate_low" => isset($_POST["estimate_low"])
            ? floatval(
                str_replace(
                    [" ", ","],
                    ["", "."],
            ...
            : null,
        "avis2_estimate_high" => isset($_POST["estimate_high"])
            ? floatval(
                str_replace(
                    [" ", ","],
                    ["", "."],
            ...
            : null,
        "avis2_prix_reserve" => isset($_POST["prix_reserve"])
            ? floatval(
                str_replace(
                    [" ", ","],
                    ["", "."],
    ...
    if (isset($_POST["avis_text"])) {
        $data["second_opinion"] = sanitize_textarea_field(
            wp_unslash($_POST["avis_text"]),
        );
    }
    if (isset($_POST["avis_titre"])) {
        $data["avis2_titre"] = sanitize_text_field(
            wp_unslash($_POST["avis_titre"]),
        );
    }
    if (isset($_POST["avis_dimension"])) {
        $data["avis2_dimension"] = sanitize_text_field(
            wp_unslash($_POST["avis_dimension"]),
        );
    }
    $wpdb->update(
...
function lmd_ajax_get_estimation_tag_display($id, $opinion)
{
    ...
    $tags_by_type = function_exists("lmd_build_tags_by_type")
        ? lmd_build_tags_by_type($linked_tags, $opinion)
        : [];
    $estimation_tag = $tags_by_type["estimation"] ?? null;
    $ai = !empty($estimation->ai_analysis)
        ? (json_decode($estimation->ai_analysis, true) ?:
        [])
        : [];
    $estimation_source = function_exists("lmd_get_estimation_source")
        ? lmd_get_estimation_source($estimation, $ai, $estimation_tag, $opinion)
        : ["source" => "", "slug" => null, "name" => ""];
    if (!$estimation_source["slug"]) {
        return null;
    }
    $label = "Estimation";
    ...
    $colors = function_exists("lmd_get_tag_filter_colors")
        ? lmd_get_tag_filter_colors(
            "estimation",
            $estimation_source["slug"],
            $source,
        )
        : ["border" => "#e5e7eb"];
...
function lmd_ajax_create_vente()
{
    ...
    $theme_vente_slug = isset($_POST["theme_vente_slug"])
        ? sanitize_text_field($_POST["theme_vente_slug"])
        : "";
    if (!$name || !$date) {
    ...
    while (
        $wpdb->get_var(
            $wpdb->prepare(
                "SELECT id FROM $t WHERE site_id = %d AND type = 'date_vente' AND slug = %s",
                $site_id,
                $slug,
            ),
        )
    ...
    if (
        in_array("theme_vente_slug", $tag_cols, true) &&
        $theme_vente_slug !== ""
    ) {
...
function lmd_ajax_update_vente()
{
    ...
    while (
        $wpdb->get_var(
            $wpdb->prepare(
                "SELECT id FROM $t WHERE site_id = %d AND type = 'date_vente' AND slug = %s AND id != %d",
                $site_id,
                $new_slug,
                $tag_id,
            ),
    ...
    $wpdb->update(
        $t,
        $up,
        ["id" => $tag_id, "site_id" => $site_id, "type" => "date_vente"],
        null,
        ["%d", "%d", "%s"],
    );
...
function lmd_ajax_list_ventes()
{
    ...
    foreach ($opts as $o) {
        ...
        if (!empty($o->theme_vente_slug)) {
            ...
            $item["theme_vente_name"] = function_exists(
    ...
    foreach ($hotel_rows as $hr) {
        $ymd = isset($hr->sale_date)
            ? substr((string) $hr->sale_date, 0, 10)
            : "";
        if ($ymd !== "") {
            $hotel_calendar[] = [
                "date" => $ymd,
                "title" => (string) $hr->title,
            ];
        }
    }
...
function lmd_ajax_hotel_vente_add()
{
    ...
    $date = isset($_POST["sale_date"])
        ? sanitize_text_field(wp_unslash($_POST["sale_date"]))
        : "";
    ...
    if (!$date || !$title) {
        wp_send_json_error([
            "message" => __("Date et intitulé requis.", "lmd-apps-ia"),
        ]);
    }
    $db = new LMD_Database();
    $id = $db->insert_hotel_vente_row(
        $date,
        $title,
    ...
    if (!$id) {
        wp_send_json_error([
            "message" => __("Enregistrement impossible.", "lmd-apps-ia"),
        ]);
    }
...
function lmd_ajax_hotel_vente_delete()
{
    ...
    if (!$db->delete_hotel_vente_row($id)) {
        wp_send_json_error([
            "message" => __("Suppression impossible.", "lmd-apps-ia"),
        ]);
    }
    wp_send_json_success();
}
...
function lmd_ajax_report_ai_error()
{
    ...
    $user_explanation = isset($_POST["user_explanation"])
        ? sanitize_textarea_field(wp_unslash($_POST["user_explanation"]))
        : "";
    ...
    $err_cols =
        $wpdb->get_var("SHOW TABLES LIKE '$err_table'") === $err_table
            ? $wpdb->get_col("DESCRIBE $err_table")
            : [];
    ...
    if (is_multisite()) {
        ...
        if (
            $wpdb->get_var("SHOW TABLES LIKE '$remontee_table'") ===
            $remontee_table
        ) {
            $wpdb->insert(
                $remontee_table,
    ...
    $ai_est_str =
        $ai_est["low"] !== null
            ? number_format($ai_est["low"], 0, ",", " ") .
                " – " .
                number_format($ai_est["high"] ?? $ai_est["low"], 0, ",", " ") .
                " €"
            : "-";

    $body =
        "Un utilisateur a signalé que l'IA se trompe complètement sur ce lot.\n\n";
    if (!empty($user_explanation)) {
        $body .=
            "Explication de l'utilisateur :\n" . $user_explanation . "\n\n";
    }
    ...
    if (is_multisite()) {
        ...
        foreach ($super_admins as $login) {
            ...
            if (
                $u &&
                $u->user_email &&
                !in_array($u->user_email, $emails, true)
            ) {
                $emails[] = $u->user_email;
            }
        }
...
function lmd_ajax_save_analysis_pricing()
{
    ...
    $free = isset($_POST["free_granted"])
        ? max(0, (int) $_POST["free_granted"])
        : 20;
    $tiers_raw =
    ...
    foreach (array_slice($tiers_raw, 0, 5) as $t) {
        ...
        $price = isset($t["price"])
            ? max(0, (float) str_replace(",", ".", $t["price"]))
            : 0.5;
        $tiers[] = ["min_paid" => $min, "price" => $price];
    }
    usort($tiers, function ($a, $b) {
        return $a["min_paid"] - $b["min_paid"];
    });
    if (empty($tiers)) {
        $tiers = [
            ["min_paid" => 0, "price" => 0.5],
...
function lmd_ajax_log_activity()
{
    ...
    $page_type = isset($_POST["page_type"])
        ? sanitize_key($_POST["page_type"])
        : "";
    $estimation_id = isset($_POST["estimation_id"])
        ? absint($_POST["estimation_id"])
        : null;
    $duration = isset($_POST["duration"])
        ? min(60, max(0, (int) $_POST["duration"]))
        : 0;
    ...
    $wpdb->insert(
        ...
        [
            ...
            "page_type" => $page_type,
            "estimation_id" =>
                $page_type === "detail" && $estimation_id
                    ? $estimation_id
                    : null,

...
if (isset($_POST["lmd_save_api"]) && check_admin_referer("lmd_api_config")) {
    update_option(
        "lmd_gemini_key",
        sanitize_text_field($_POST["lmd_gemini_key"] ?? ""),
    );
    update_option(
        "lmd_gemini_model",
    ...
    update_option(
        "lmd_firecrawl_key",
        sanitize_text_field($_POST["lmd_firecrawl_key"] ?? ""),
    );
    update_option(
        "lmd_imgbb_key",
        sanitize_text_field($_POST["lmd_imgbb_key"] ?? ""),
    );
...
$form_action = $lmd_suite_embed
    ? admin_url("admin.php?page=lmd-apps-ia&hub_tab=apis")
    : "";
?>
...
<?php endif; ?>
    ...
    <form method="post" action="<?php echo $lmd_suite_embed
        ? esc_url($form_action)
        : ""; ?>" class="lmd-api-config-form">
        <?php wp_nonce_field("lmd_api_config"); ?>

        <div class="lmd-api-config-field">
            <span class="lmd-api-config-field-label"><?php esc_html_e(
                "Clé Gemini (Google AI)",
                "lmd-apps-ia",
            ); ?></span>
            <div class="lmd-api-config-field-row">
                <input type="password" name="lmd_gemini_key" id="lmd_gemini_key" value="<?php echo esc_attr(
                    $gemini,
                ); ?>" class="regular-text lmd-api-config-input" autocomplete="off" />
                <span class="lmd-api-config-field-links">
                    <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener"><?php esc_html_e(
        ...
        <div class="lmd-api-config-field">
            <span class="lmd-api-config-field-label" id="lmd-label-gemini-model"><?php esc_html_e(
                "Modèle Gemini",
                "lmd-apps-ia",
            ); ?></span>
            <div class="lmd-api-config-field-row">
                <input type="text" name="lmd_gemini_model" id="lmd_gemini_model" value="<?php echo esc_attr(
                    $gemini_model,
                ); ?>" class="regular-text lmd-api-config-input" placeholder="gemini-2.5-pro" aria-labelledby="lmd-label-gemini-model" />
                <span class="lmd-api-config-field-links">
                    <?php esc_html_e(
        ...
        <div class="lmd-api-config-field">
            <span class="lmd-api-config-field-label"><?php esc_html_e(
                "Clé SerpAPI (Google Lens)",
                "lmd-apps-ia",
            ); ?></span>
            <div class="lmd-api-config-field-row">
                <input type="password" name="lmd_serpapi_key" id="lmd_serpapi_key" value="<?php echo esc_attr(
                    $serp,
                ); ?>" class="regular-text lmd-api-config-input" autocomplete="off" />
                <span class="lmd-api-config-field-links">
                    <?php esc_html_e(
                        "Recherche par image type Google Lens",
                        "lmd-apps-ia",
                    ); ?> —
                    <a href="https://serpapi.com/manage-api-key" target="_blank" rel="noopener"><?php esc_html_e(
        ...
        <div class="lmd-api-config-field">
            ...
            <div class="lmd-api-config-field-row">
                <input type="password" name="lmd_firecrawl_key" id="lmd_firecrawl_key" value="<?php echo esc_attr(
                    $firecrawl,
                ); ?>" class="regular-text lmd-api-config-input" autocomplete="off" />
                <span class="lmd-api-config-field-links">
                    <?php esc_html_e(
                        "Scraping web pour enrichir l’analyse",
                        "lmd-apps-ia",
                    ); ?> —
                    <a href="https://www.firecrawl.dev/app/api-keys" target="_blank" rel="noopener"><?php esc_html_e(
        ...
        <div class="lmd-api-config-field lmd-api-config-field--imgbb">
            <span class="lmd-api-config-field-label"><?php esc_html_e(
                "ImgBB (images locales → URL publique)",
                "lmd-apps-ia",
            ); ?></span>
            <p class="lmd-api-config-field-hint">
                <?php printf /* translators: %s: WP environment type (local, production, etc.) */(
                    esc_html__(
                        "Utile quand SerpAPI ne peut pas lire un fichier local. Environnement WordPress actuel : %s. En production, laissez désactivé sauf besoin ponctuel.",
                        "lmd-apps-ia",
                    ),
            ...
            <p class="lmd-api-config-toggle">
                <label>
                    <input type="checkbox" name="lmd_imgbb_enabled" value="1" <?php checked(
                        $imgbb_enabled,
                    ); ?> />
                    <?php esc_html_e(
                        "Activer l’upload ImgBB (recommandé surtout en local / préproduction)",
                        "lmd-apps-ia",
                    ); ?>
                </label>
            </p>
            <div class="lmd-api-config-field-row">
                <input type="password" name="lmd_imgbb_key" id="lmd_imgbb_key" value="<?php echo esc_attr(
                    $imgbb,
                ); ?>" class="regular-text lmd-api-config-input" autocomplete="off" />
                <span class="lmd-api-config-field-links">
                    <a href="https://api.imgbb.com/" target="_blank" rel="noopener"><?php esc_html_e(
                        "Obtenir une clé ImgBB",
                        "lmd-apps-ia",
                    ); ?></a>
                </span>
            </div>
        ...
        <p class="submit"><input type="submit" name="lmd_save_api" class="button button-primary" value="<?php esc_attr_e(
            "Enregistrer",
            "lmd-apps-ia",
        ); ?>" /></p>
    </form>

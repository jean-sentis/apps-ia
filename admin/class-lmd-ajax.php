...
class LMD_Ajax {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_lmd_save_delegation_draft', [$this, 'save_delegation_draft']);
        ...
        add_action('wp_ajax_lmd_list_delegation_recipients', [$this, 'list_delegation_recipients']);
        add_action('wp_ajax_lmd_save_delegation_full', [$this, 'save_delegation_full']);
        add_action('wp_ajax_lmd_send_delegation_email', [$this, 'send_delegation_email']);
        add_action('wp_ajax_lmd_generate_delegation_token', [$this, 'generate_delegation_token']);
    ...
    public function get_category_settings() {
        ...
        if ($type === 'estimation') {
            $options = function_exists('lmd_get_custom_category_options') ? lmd_get_custom_category_options('estimation') : [];
            if (empty($options)) {
                $options = function_exists('lmd_get_default_estimation_options') ? lmd_get_default_estimation_options() : [];
            }
        } else {
            $options = function_exists('lmd_get_custom_category_options') ? lmd_get_custom_category_options('theme_vente') : [];
            if (empty($options)) {
                $options = function_exists('lmd_get_default_theme_vente_options') ? lmd_get_default_theme_vente_options() : [];
            }
        }
    ...
    public function save_category_settings() {
        ...
        foreach ($raw as $o) {
            ...
            if ($type === 'estimation') {
                $options[] = [
                    'slug' => $slug,
                    'name' => isset($o['name']) ? wp_strip_all_tags($o['name']) : $slug,
                    'max' => isset($o['max']) && $o['max'] !== '' ? floatval($o['max']) : null,
                    'min' => isset($o['min']) && $o['min'] !== '' ? floatval($o['min']) : null,
                ];
            } else {
                $options[] = [
                    'slug' => $slug,
                    'name' => isset($o['name']) ? wp_strip_all_tags($o['name']) : $slug,
                    'parent_slug' => isset($o['parent_slug']) ? sanitize_text_field($o['parent_slug']) : '',
                ];
            }
        }
        if ($type === 'estimation') {
            $options = array_filter($options, function ($o) {
                return isset($o['max']) && $o['max'] !== null || isset($o['min']) && $o['min'] !== null;
            });
        }
    ...
    public function save_avis_draft() {
        ...
        $wpdb->update($wpdb->prefix . 'lmd_estimations', [$col => $text], ['id' => $id], ['%s'], ['%d']);
    ...
    public function save_estimates() {
        ...
        if (isset($_POST['avis1_estimate_low'])) {
            $data['avis1_estimate_low'] = floatval(str_replace([' ', ','], ['', '.'], $_POST['avis1_estimate_low']));
        }
        if (isset($_POST['avis1_estimate_high'])) {
            $data['avis1_estimate_high'] = floatval(str_replace([' ', ','], ['', '.'], $_POST['avis1_estimate_high']));
        }
        if (isset($_POST['avis1_prix_reserve'])) {
            $data['avis1_prix_reserve'] = floatval(str_replace([' ', ','], ['', '.'], $_POST['avis1_prix_reserve']));
        }
        if (isset($_POST['avis2_estimate_low'])) {
            $data['avis2_estimate_low'] = floatval(str_replace([' ', ','], ['', '.'], $_POST['avis2_estimate_low']));
        }
        if (isset($_POST['avis2_estimate_high'])) {
            $data['avis2_estimate_high'] = floatval(str_replace([' ', ','], ['', '.'], $_POST['avis2_estimate_high']));
        }
        if (isset($_POST['avis2_prix_reserve'])) {
            $data['avis2_prix_reserve'] = floatval(str_replace([' ', ','], ['', '.'], $_POST['avis2_prix_reserve']));
        }
        if (!empty($data)) {
    ...
    public function set_tag_by_slug() {
        ...
        if (empty($slug)) {
            $wpdb->query($wpdb->prepare("DELETE et FROM $et_table et INNER JOIN $tags_table t ON et.tag_id = t.id WHERE et.estimation_id = %d AND t.type = %s", $id, $type));
            wp_send_json_success(['tag_id' => 0]);
        }

        $tag = $wpdb->get_row($wpdb->prepare("SELECT id, name FROM $tags_table WHERE site_id = %d AND type = %s AND slug = %s", $site_id, $type, $slug));
        if (!$tag) {
            $cats = function_exists('lmd_get_tag_categories') ? lmd_get_tag_categories() : [];
            $name = $slug;
            ...
            $wpdb->insert($tags_table, ['site_id' => $site_id, 'name' => $name, 'type' => $type, 'slug' => $slug], ['%d', '%s', '%s', '%s']);
        ...
        $wpdb->query($wpdb->prepare("DELETE et FROM $et_table et INNER JOIN $tags_table t ON et.tag_id = t.id WHERE et.estimation_id = %d AND t.type = %s", $id, $type));
        $wpdb->insert($et_table, ['estimation_id' => $id, 'tag_id' => $tag_id, 'modified_by_avis' => $opinion], ['%d', '%d', '%d']);
    ...
    public function save_delegation_draft() {
        ...
        $id = isset($_POST['id']) ? absint($_POST['id']) : 0;
        $draft = isset($_POST['draft']) ? wp_kses_post(wp_unslash($_POST['draft'])) : '';
        $email = isset($_POST['email']) ? sanitize_email($_POST['email']) : '';
        if (!$id) {
            wp_send_json_error(['message' => 'ID manquant']);
        }
        global $wpdb;
        $wpdb->update($wpdb->prefix . 'lmd_estimations', ['delegation_draft' => $draft, 'delegation_email' => $email], ['id' => $id], ['%s', '%s'], ['%d']);
        wp_send_json_success();
    }
    ...
    public function get_cp_settings() {
        ...
        if ($row && !empty($row->cp_copy_emails)) {
            $copy = array_filter(array_map('trim', explode(',', wp_unslash($row->cp_copy_emails))));
        }
        wp_send_json_success([
            'email' => $row->cp_email ?? '',
    ...
    public function save_cp_settings() {
        ...
        $copy = isset($_POST['copy_emails']) ? sanitize_text_field(wp_unslash($_POST['copy_emails'])) : '';
        global $wpdb;
        $user_id = get_current_user_id();
        $site_id = get_current_blog_id();
        ...
        if ($exists) {
            $wpdb->update($table, ['cp_email' => $email, 'cp_signature' => $signature, 'cp_copy_emails' => $copy], ['site_id' => $site_id, 'user_id' => $user_id], ['%s', '%s', '%s'], ['%d', '%d']);
        } else {
            $wpdb->insert($table, ['site_id' => $site_id, 'user_id' => $user_id, 'cp_email' => $email, 'cp_signature' => $signature, 'cp_copy_emails' => $copy], ['%d', '%d', '%s', '%s', '%s']);
        }
        wp_send_json_success();
    }
    ...
    public function save_formule() {
        ...
        if ($id) {
            $wpdb->update($table, ['name' => $name, 'content' => $content], ['id' => $id, 'user_id' => $user_id], ['%s', '%s'], ['%d', '%d']);
            wp_send_json_success(['id' => $id]);
        } else {
            $wpdb->insert($table, ['site_id' => $site_id, 'user_id' => $user_id, 'name' => $name, 'content' => $content], ['%d', '%d', '%s', '%s']);
    ...
    public function delete_formule() {
        ...
        global $wpdb;
        $user_id = get_current_user_id();
        $wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}lmd_formules WHERE id = %d AND user_id = %d", $id, $user_id));
        wp_send_json_success();
    }
    ...
    public function save_delegation_full() {
        ...
        if ($email) {
            ...
            $site_id = get_current_blog_id();
            $exists = $wpdb->get_var($wpdb->prepare("SELECT id FROM $table WHERE site_id = %d AND email = %s", $site_id, $email));
            if (!$exists) {
                $wpdb->insert($table, ['site_id' => $site_id, 'email' => $email], ['%d', '%s']);
            }
        }
        wp_send_json_success();
    }
    ...
    public function send_delegation_email() {
        ...
        $body_html = preg_replace_callback(
            ...
            function ($m) {
                return '<a href="' . esc_url($m[1]) . '">à cliquer pour rejoindre la demande d\'estimation</a>';
            },
            $body_html
        );
    ...
    public function generate_delegation_token() {
        ...
        global $wpdb;
        $wpdb->insert($wpdb->prefix . 'lmd_delegation_tokens', [
            'estimation_id' => $id,
            'token' => $token,
            'email' => $email ?: '',
            'expires_at' => $expires,
        ...
        $admin_url = admin_url('admin.php?page=lmd-estimation-detail&id=' . $id . '&token=' . $token);
        wp_send_json_success(['url' => $public_url, 'admin_url' => $admin_url, 'token' => $token]);
    }
    ...
    public function save_reponse() {
        ...
        $questions = isset($_POST['questions_selected']) && is_array($_POST['questions_selected']) ? array_map('absint', $_POST['questions_selected']) : [];
        global $wpdb;
        ...
        if ($mark_sent) {
            $site_id = get_current_blog_id();
            $tag = $wpdb->get_row($wpdb->prepare("SELECT id FROM {$wpdb->prefix}lmd_tags WHERE site_id = %d AND type = 'message' AND slug = 'repondu'", $site_id));
            if ($tag) {
                $wpdb->query($wpdb->prepare("DELETE et FROM {$wpdb->prefix}lmd_estimation_tags et INNER JOIN {$wpdb->prefix}lmd_tags t ON et.tag_id = t.id WHERE et.estimation_id = %d AND t.type = 'message'", $id));
                $wpdb->insert($wpdb->prefix . 'lmd_estimation_tags', ['estimation_id' => $id, 'tag_id' => $tag->id], ['%d', '%d']);
            }
        }
        wp_send_json_success(['sent_at' => $mark_sent ? ($data['reponse_sent_at'] ?? null) : null]);
    }

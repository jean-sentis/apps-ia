...
class LMD_Activity_Analytics
{
    ...
    public function get_monthly_stats($year_from = null, $year_to = null)
    {
        ...
        if ($this->wpdb->get_var("SHOW TABLES LIKE '$log'") === $log) {
            ...
            $log_rows = $this->wpdb->get_results(
                $this->wpdb->prepare(
            ...
            foreach ($log_rows ?: [] as $r) {
                if (!isset($months[$r->month])) {
                    $months[$r->month] = ["estimations" => 0];
                }
                $key =
                    $r->page_type === "detail"
                        ? "detail_seconds"
    ...
    public function get_per_estimation_stats($month = null)
    {
        ...
        $select =
            "e.id, e.created_at, e.first_viewed_at, e.status, e.ai_analysis, e.avis1_estimate_low, e.avis1_estimate_high";
        if ($has_ai_launch) {
            $select .= ", e.ai_launch_count";
        }
        ...
        foreach ($rows ?: [] as $r) {
            ...
            if (
                $has_first_viewed &&
                !empty($r->first_viewed_at) &&
                !empty($r->created_at)
            ) {
                $delay_hours =
                    (strtotime($r->first_viewed_at) -
            ...
            $detail_sec = 0;
            if ($this->wpdb->get_var("SHOW TABLES LIKE '$log'") === $log) {
                $detail_sec = (int) $this->wpdb->get_var(
                    $this->wpdb->prepare(
            ...
            if (!empty($r->ai_analysis)) {
                ...
                $ai_est = function_exists("lmd_get_ai_estimation")
                    ...
                    : ["low" => null, "high" => null];
                $ai_interet = trim(
                    $ai["interest_level"] ?? ($ai["interet"] ?? ""),
                );
                if (
                    $ai_est["low"] !== null &&
                    $r->avis1_estimate_low !== null
                ) {
                    $diff = abs(
                        ($r->avis1_estimate_low + $r->avis1_estimate_high) / 2 -
                            ($ai_est["low"] +
                                ($ai_est["high"] ?? $ai_est["low"])) /
                                2,
                    );
                    $ai_follows_price =
                        $diff < 50 ? "oui" : ($diff < 200 ? "proche" : "non");
                }
                $interet_tag = $this->wpdb->get_var(
                    $this->wpdb->prepare(
                ...
                if ($ai_interet && $interet_tag) {
                    ...
                    $ai_follows_interest =
                        $norm($interet_tag) === $norm($ai_interet)
                            ? "oui"
                            : "non";
                }
            }
    ...
    public function get_month_aggregates($month)
    {
        ...
        $delay_hours = array_filter(
            array_column($stats, "delay_hours"),
            function ($v) {
                return $v !== null;
            },
        );
        ...
        $follows_interest = array_filter(
            array_column($stats, "ai_follows_interest"),
        );

        $log = $this->prefix . "activity_log";
        $grid_sec = 0;
        if (
            $this->wpdb->get_var("SHOW TABLES LIKE '$log'") === $log &&
            $month
        ) {
            $grid_sec = (int) $this->wpdb->get_var(
                $this->wpdb->prepare(
                    "SELECT COALESCE(SUM(duration_seconds), 0) FROM $log WHERE site_id = %d AND page_type = 'grid' AND created_at >= %s AND created_at < %s",
                    $this->site_id,
                    $month . "-01 00:00:00",
                    date("Y-m-d", strtotime($month . "-01 +1 month")) .
                        " 00:00:00",
                ),
            );
        }
        ...
        return [
            "count" => count($stats),
            "delay_avg_hours" => !empty($delay_hours)
                ? array_sum($delay_hours) / count($delay_hours)
                : null,
            "detail_total_minutes" => round($detail_sec / 60, 1),
            "grid_total_minutes" => round($grid_sec / 60, 1),
            "ai_launch_total" => $ai_launches,
            "follows_price_oui" => count(
                array_filter($follows_price, function ($v) {
                    return $v === "oui";
                }),
            ),
            "follows_price_total" => count($follows_price),
            "follows_interest_oui" => count(
                array_filter($follows_interest, function ($v) {
                    return $v === "oui";
                }),
            ),
    ...
    public function get_feature_usage($month = null, $all_sites = false)
    {
        ...
        foreach ($sites as $sid) {
            $switched_blog = false;
            if (is_multisite() && $sid !== $current_blog_id) {
                switch_to_blog($sid);
                $switched_blog = true;
            }
            ...
            if ($this->wpdb->get_var("SHOW TABLES LIKE '$f_t'") === $f_t) {
                ...
                if ($month) {
                    ...
                    if (
                        !empty($f_cols) &&
                        in_array("created_at", $f_cols, true)
                    ) {
        ...
        $least_used = array_slice(
            array_filter($least_used, function ($k) use ($by_feature) {
        ...
        foreach ($sites as $sid) {
            ...
            if ($has_table) {
                $users_with = (int) $this->wpdb->get_var(
                    "SELECT COUNT(*) FROM $cp WHERE site_id = " .
                        (int) $sid .
                        " AND cp_signature IS NOT NULL AND TRIM(cp_signature) != ''",
                );
            }
            $result[] = [
                "site_id" => $sid,

...
class LMD_Full_Export_Import {
    ...
    private static $TABLES = [
        'estimations', 'tags', 'estimation_tags', 'pricing_tiers', 'client_pricing_overrides',
        'api_usage', 'ai_error_reports', 'cp_settings', 'formules', 'delegation_recipients',
        'delegation_tokens', 'activity_log',
    ];
     ...
     */
    public function export($site_id = null, $include_photos = true, $include_api_keys = false) {
        ...
        $tmp_dir = $upload_dir['basedir'] . '/lmd-export-' . wp_generate_password(8, false);
        wp_mkdir_p($tmp_dir);
        ...
        foreach (self::$TABLES as $base) {
            ...
            $rows = $this->wpdb->get_results("SELECT * FROM $table WHERE site_id = " . (int) $site_id, ARRAY_A);
            if (in_array($base, ['estimation_tags', 'delegation_tokens'], true)) {
                $rows = $this->wpdb->get_results("SELECT * FROM $table", ARRAY_A);
            }
            if ($base === 'delegation_tokens') {
                $est_ids = array_unique(array_column($rows, 'estimation_id'));
                $est_in_site = $this->wpdb->get_col("SELECT id FROM {$prefix}estimations WHERE site_id = " . (int) $site_id);
                $est_in_site = array_flip($est_in_site);
                $rows = array_filter($rows, function ($r) use ($est_in_site) {
                    return isset($est_in_site[$r['estimation_id']]);
                });
            }
            if ($base === 'estimation_tags') {
                $tag_ids = $this->wpdb->get_col("SELECT id FROM {$prefix}tags WHERE site_id = " . (int) $site_id);
                $est_ids = $this->wpdb->get_col("SELECT id FROM {$prefix}estimations WHERE site_id = " . (int) $site_id);
                $rows = array_filter($rows, function ($r) use ($tag_ids, $est_ids) {
                    return in_array($r['tag_id'], $tag_ids) && in_array($r['estimation_id'], $est_ids);
                });
            }
            $data['tables'][$base] = $rows;
        }

        foreach (array_merge(self::$OPTIONS, $include_api_keys ? self::$OPTIONS_SENSITIVE : []) as $opt) {
        ...
        if (is_multisite() && $site_id !== get_current_blog_id()) {
            switch_to_blog(1);
            foreach (['lmd_consumption_monthly_email', 'lmd_consumption_monthly_enabled'] as $opt) {
                $val = get_option($opt, '__NOT_SET__');
                if ($val !== '__NOT_SET__') {
                    $data['options'][$opt] = $val;
                }
        ...
        if ($include_photos && !empty($data['tables']['estimations'])) {
            ...
            foreach ($data['tables']['estimations'] as $i => $est) {
                ...
                if (!empty($est['photos'])) {
                    ...
                    $photos = is_array($decoded) ? $decoded : (is_string($est['photos']) ? [$est['photos']] : []);
                }
                $new_urls = [];
                ...
                $data['tables']['estimations'][$i]['photos'] = !empty($new_urls) ? wp_json_encode($new_urls) : $est['photos'];
            }
            $data['photo_map'] = $photo_map;
        }

        file_put_contents($tmp_dir . '/data.json', wp_json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));

        $zip_path = $upload_dir['basedir'] . '/lmd-copie-client-' . date('Y-m-d-His') . '.zip';
        $zip = new ZipArchive();
    ...
    public function import($zip_path, $replace = false) {
        $upload = wp_upload_dir();
        $tmp_dir = $upload['basedir'] . '/lmd-import-' . wp_generate_password(8, false);
        wp_mkdir_p($tmp_dir);
            ...
            foreach (['estimation_tags', 'estimations', 'tags'] as $t) {
                ...
                if ($this->wpdb->get_var("SHOW TABLES LIKE '$table'") === $table) {
                    if ($t === 'estimation_tags') {
                        $this->wpdb->query("DELETE et FROM $table et INNER JOIN {$prefix}tags t ON et.tag_id = t.id WHERE t.site_id = " . (int) $site_id);
                        $this->wpdb->query("DELETE et FROM $table et INNER JOIN {$prefix}estimations e ON et.estimation_id = e.id WHERE e.site_id = " . (int) $site_id);
                    } else {
                        $this->wpdb->delete($table, ['site_id' => $site_id], ['%d']);
                    }
                }
            }
        }
        ...
        if (!empty($data['tables']['estimation_tags']) && !empty($est_id_map)) {
            foreach ($data['tables']['estimation_tags'] as $row) {
                ...
                if ($new_est && $new_tag) {
                    $this->wpdb->replace($prefix . 'estimation_tags', [
        ...
        foreach (['pricing_tiers', 'client_pricing_overrides', 'cp_settings', 'formules', 'delegation_recipients'] as $t) {
            if (empty($data['tables'][$t])) continue;
            $table = $prefix . $t;
            if ($this->wpdb->get_var("SHOW TABLES LIKE '$table'") !== $table) continue;
        ...
        foreach (['api_usage', 'ai_error_reports', 'activity_log'] as $t) {
            ...
            foreach ($data['tables'][$t] as $row) {
                unset($row['id']);
                $row['site_id'] = $site_id;
                if (isset($row['estimation_id']) && isset($est_id_map[$row['estimation_id']])) {
        ...
        if (!empty($data['tables']['delegation_tokens']) && !empty($est_id_map)) {
    ...
    public function get_exportable_sites() {
        ...
        foreach (get_sites(['number' => 500]) as $s) {
            $sites[] = ['id' => (int) $s->blog_id, 'name' => get_blog_option($s->blog_id, 'blogname', 'Site ' . $s->blog_id)];
        }
        return $sites;
    }
}

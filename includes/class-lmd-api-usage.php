...
class LMD_Api_Usage
{
    ...
    private static $PRICING_TIERS = [
        ...
        "imgbb" => [["min" => 0, "max" => 999999, "price" => 0]],
        "gemini" => [
    ...
    /**
     * Synchronise l'objet avec le blog WordPress courant.
     */
    private function sync_context()
    {
        global $wpdb;
        $this->wpdb = $wpdb;
    ...
    /**
     * Nom de table de suivi pour le blog courant.
     */
    private function get_api_usage_table()
    {
        $this->sync_context();
        return $this->prefix . "api_usage";
    }
     ...
     * @return mixed
     */
    private function with_site_context($site_id, callable $callback)
    {
        $current_site = get_current_blog_id();
        $target_site = $site_id !== null ? (int) $site_id : $current_site;
        $switched = is_multisite() && $target_site !== $current_site;
    ...
    private function estimate_cost_for_units($api_name, $units, $site_id)
    {
        ...
        $price_per_unit = $this->get_price_for_volume(
            $api_name,
            $current + $units,
        );
        return $price_per_unit !== null
            ? round($price_per_unit * $units, 6)
            : null;
    }
    ...
    public function get_price_for_volume($api_name, $monthly_volume)
    {
        ...
        foreach ($tiers as $t) {
            if (
                $monthly_volume >= $t["min"] &&
                ($t["max"] === null || $monthly_volume <= $t["max"])
            ) {
                return $t["price"];
            }
        }
    ...
    {
        $tiers = self::$PRICING_TIERS[$api_name] ?? [];
        $current_price = $this->get_price_for_volume(
            $api_name,
            $current_volume,
        );
        foreach ($tiers as $t) {
    ...
    {
        $target_site = $site_id ?? get_current_blog_id();

        return $this->with_site_context($target_site, function (
            $resolved_site_id,
        ) {
            $this->ensure_table_exists();
            $table = $this->get_api_usage_table();
    ...
    public static function get_request_price_tiers()
    {
        $tiers = [
            ...
            [
                "min" => 501,
                "max" => 1000,
                "label" => "501 à 1 000 requêtes/mois",
            ],
            [
                "min" => 1001,
                "max" => 3000,
                "label" => "1 001 à 3 000 requêtes/mois",
            ],
            [
                "min" => 3001,
                "max" => 5000,
                "label" => "3 001 à 5 000 requêtes/mois",
            ],
            [
                "min" => 5001,
                "max" => null,
                "label" => "Au-delà de 5 000 requêtes/mois",
            ],
        ];
        $usage = new self();
        foreach ($tiers as &$t) {
            $n =
                $t["max"] !== null ? (int) (($t["min"] + $t["max"]) / 2) : 7500;
            $serp_vol = $n;
            $fire_vol = $n * 3;
            $gemini_vol = $n;
     ...
     */
    public function get_consumption_for_period(
        $site_id,
        $month_start,
        $month_end,
    ) {
        return $this->with_site_context($site_id, function (
            $resolved_site_id,
        ) use ($month_start, $month_end) {
            $this->ensure_table_exists();
            $table = $this->get_api_usage_table();
            ...
            $analyses_count = (int) $this->wpdb->get_var(
                $this->wpdb->prepare(
    ...
    public function get_all_clients_consumption($month, $all_clients = true)
    {
        ...
        if (is_multisite() && $all_clients) {
            ...
            foreach ($sites as $site) {
                ...
                $data = $this->get_consumption_for_period(
                    $site_id,
                    $month_start,
                    $month_end,
                );
            ...
            $out[] = array_merge(
                [
                    "site_id" => $site_id,
                    "site_name" => get_bloginfo("name") ?: "Site " . $site_id,
                ],
                $data,
            );
        }
    ...
    public function get_aggregate_consumption($month, $all_clients = true)
    {
        ...
        $by_api = [
            "serpapi" => ["units" => 0, "cost_usd" => 0],
            "firecrawl" => ["units" => 0, "cost_usd" => 0],
            "imgbb" => ["units" => 0, "cost_usd" => 0],
            "gemini" => ["units" => 0, "cost_usd" => 0],
        ];
        $total_usd = 0;
        $analyses_count = 0;
        $services = [
    ...
    public static function get_analysis_pricing_tiers($site_id = null)
    {
        ...
        if (!empty($tiers_raw) && is_array($tiers_raw)) {
            foreach ($tiers_raw as $t) {
                ...
                $price = isset($t["price"]) ? (float) $t["price"] : 0.5;
     ...
     * @return array
     */
    public function get_consumption_billing_summary(
        $site_id = null,
        $month_ym = null,
    ) {
        $target_site = $site_id ?? get_current_blog_id();
        ...
        return $this->with_site_context($target_site, function (
            ...
            $this->ensure_table_exists();
            $table = $this->get_api_usage_table();
            $cfg = self::get_analysis_pricing_tiers($resolved_site_id);
            ...
            $total_analyses = (int) $this->wpdb->get_var(
                $this->wpdb->prepare(
            ...
            $analyses_before_month = (int) $this->wpdb->get_var(
                $this->wpdb->prepare(
            ...
            $analyses_this_month = (int) $this->wpdb->get_var(
                $this->wpdb->prepare(
            ...
            $free_remaining_for_month = max(
                0,
                $free_granted - $free_before_month,
            );
            $free_this_month = min(
                $free_remaining_for_month,
                $analyses_this_month,
            );
            $paid_this_month = max(0, $analyses_this_month - $free_this_month);
            $paid_before_month = max(
                0,
                $analyses_before_month - $free_before_month,
            );

            $amount_ht_this_month = 0;
            for ($i = 0; $i < $paid_this_month; $i++) {
                $rank = $paid_before_month + $i;
                $amount_ht_this_month += self::get_price_for_paid_rank(
                    $rank,
                    $resolved_site_id,
                );
            }
            ...
            $price_per_paid =
                $paid_this_month > 0
                    ? round($amount_ht_this_month / $paid_this_month, 2)
                    : self::get_price_for_paid_rank(0, $resolved_site_id);
            ...
            $summary = [
                ...
                "pricing_tiers" => self::get_analysis_pricing_tiers(
                    $resolved_site_id,
                ),
            ];
     ...
     * @return array
     */
    public function get_parent_product_margin_report(
        $month_ym,
        $all_clients = true,
    ) {
        if (!preg_match('/^\d{4}-\d{2}$/', (string) $month_ym)) {
        ...
        if (is_multisite() && $all_clients) {
            ...
            foreach ($sites as $site) {
                ...
                $sites_detail[] = [
                    "site_id" => $sid,
                    "site_name" =>
                        get_blog_option($sid, "blogname", "") ?: "Site " . $sid,
                    "revenue_eur" => round(
                        (float) $s["amount_ht_this_month"],
                        2,
                    ),
        ...
        $margin_pct =
            $revenue_eur > 0
                ? round((100 * $margin_eur) / $revenue_eur, 1)
                : null;

        $avg_cost_per_analysis_eur =
            $qty > 0 ? round($cost_eur / $qty, 4) : 0.0;
        $avg_price_per_paid_eur =
            $paid_sum > 0 ? round($revenue_eur / $paid_sum, 4) : 0.0;
        $avg_revenue_per_analysis_eur =
            $qty > 0 ? round($revenue_eur / $qty, 4) : 0.0;

...
class LMD_Estimation_Processor
{
    ...
    public function run_analysis($estimation_id)
    {
        ...
        $image_url_public = $this->get_serpapi_image_url(
            $photos,
            $estimation_id,
        );
        if (
            $image_url_public &&
            $this->api &&
            method_exists($this->api, "call_serpapi_lens")
        ) {
            $this->set_progress($estimation_id, 20, "Recherche Google Lens...");
            $lens_query = $this->enrich_lens_query($description);
            $lens_data = $this->api->call_serpapi_lens(
                $image_url_public,
                $lens_query,
                "all",
            );
            $this->log_api_usage("serpapi", 1, $estimation_id);
            $lens_matches = $this->extract_lens_matches($lens_data);
            if (empty($lens_matches) && !empty($lens_query)) {
                $lens_data = $this->api->call_serpapi_lens(
                    $image_url_public,
                    "",
                    "all",
            ...
            $this->set_progress(
                $estimation_id,
                20,
                "Pas de Lens (image locale ou sans clé)",
            );
        }
        ...
        $user_explanation = get_transient(
            "lmd_ai_error_explanation_" . $estimation_id,
        );
        if ($user_explanation) {
            delete_transient("lmd_ai_error_explanation_" . $estimation_id);
        }
        $ai_output = $this->run_gemini_analysis(
            $description,
            $photos,
            $lens_data,
            $scraped_content,
        ...
        $ai_output = $this->enrich_ai_output(
            $ai_output,
            $lens_data,
            $scraped_content,
        );
        ...
        $up = [
            "status" => "ai_analyzed",
            "ai_analysis" => wp_json_encode($ai_output),
        ];
    ...
    /**
     * Extrait une liste de correspondances Lens exploitable depuis les différents blocs de réponse.
     */
    private function extract_lens_matches($lens_data)
    {
        ...
        foreach ($groups as $key => $match_type) {
            $items = $lens_data[$key] ?? [];
            if (!is_array($items)) {
                continue;
            }
            foreach ($items as $item) {
                $normalized = $this->normalize_lens_match_item(
                    $item,
                    $match_type,
                );
                if (!$normalized) {
    ...
    private function normalize_lens_match_item($item, $match_type)
    {
        if (!is_array($item)) {
            return null;
        }
    ...
    private function pick_first_url($values)
    {
        foreach ((array) $values as $value) {
            if (is_array($value)) {
                $nested = $this->pick_first_url($value);
                if ($nested !== "") {
                    return $nested;
                }
                continue;
    ...
    {
        set_transient(
            "lmd_analysis_progress_" . $estimation_id,
            [
    ...
    private function get_first_photo_public_url($photos)
    {
        ...
        $first = is_array($photos[0])
            ? $photos[0]["url"] ??
                ($photos[0]["path"] ?? ($photos[0]["file"] ?? ""))
            : $photos[0];
        ...
        $fullpath =
            strpos($first, $basedir) === 0
                ? $first
                : $basedir . "/" . ltrim(str_replace("\\", "/", $first), "/");
    ...
    private function run_gemini_analysis(
        $description,
        $photos,
        $lens_data,
        $scraped_content,
        ...
        $prompt = $this->build_gemini_prompt(
            $description,
            $lens_data,
            $scraped_content,
            $user_explanation,
    ...
    private function build_gemini_prompt(
        $description,
        $lens_data,
        $scraped_content,
        $user_explanation = "",
        ...
        $lens_summary = "";
        $lens_matches = $this->extract_lens_matches($lens_data);
        if (!empty($lens_matches)) {
            $items = array_slice($lens_matches, 0, 8);
            $lines = [];
            foreach ($items as $i => $m) {
                $title = $m["title"] ?? "";
                $link = $m["link"] ?? "";
                $type = $m["match_type"] ?? "visual";
                $type_label =
                    $type === "exact"
                        ? "exact"
                        : ($type === "product"
            ...
            $lens_summary =
                "Résultats Google Lens (exact / visuel / produit) — Tu DOIS fournir un verdict visual_comparisons pour CHAQUE index:\n" .
                implode("\n", $lines);
        }

        $slugs_interet =
            "pas_pour_nous, peu_interessant, a_examiner, interessant, tres_interessant, exceptionnel";
        $est_opts = function_exists("lmd_get_estimation_options_merged")
            ? lmd_get_estimation_options_merged()
            : [];
        $slugs_estimation = !empty($est_opts)
            ? implode(
                ", ",
            ...
            : "moins_25, moins_100, moins_500, moins_1000, moins_5000, plus_5000";
        $theme_opts = function_exists("lmd_get_theme_vente_options_merged")
            ? lmd_get_theme_vente_options_merged()
            : [];
        $slugs_theme = !empty($theme_opts)
            ? implode(
                ", ",
                array_map(function ($o) {
                    return $o["slug"];
            ...
            : "tableaux_dessins_anciens, art_moderne, art_contemporain, arts_decoratifs_design, mobilier_objets_art, bijoux_joaillerie, mode_maroquinerie_luxe, livres_manuscrits_autographes, vins_spiritueux, vehicules_collection, arts_premiers_civilisations";
    ...
    private function normalize_ai_output($data)
    {
        ...
        $visual_comparisons = $this->normalize_visual_comparisons(
            $data["visual_comparisons"] ?? [],
        );
        $market_results = $this->normalize_market_results(
            $data["market_results"] ?? ($data["market"] ?? []),
        );
        $questions_arr = $this->normalize_questions(
            $data["questions"] ?? ($data["questions_for_owner"] ?? []),
        );
        ...
        $out = array_merge(
            ...
            array_filter([
                ...
                "theme_vente" => $this->sanitize_slug(
                    $data["theme_vente"] ?? "",
                ),
                "theme_vente_suggested_parent" => $this->sanitize_slug(
                    $data["theme_vente_suggested_parent"] ?? "",
                ),
                "sub_categories" => is_array($data["sub_categories"] ?? null)
                    ? array_map(
                        [$this, "sanitize_slug"],
                        $data["sub_categories"],
                    )
                    : [],
                "estimate_low" => $this->parse_number(
                    $data["estimate_low"] ?? null,
                ),
                "estimate_high" => $this->parse_number(
                    $data["estimate_high"] ?? null,
                ),
                "interest_level" => $data["interest_level"] ?? "",
                "reliability" => $data["reliability"] ?? "",
                "identity" => $this->ensure_string(
                    $data["identity"] ?? ($data["identity_biography"] ?? ""),
                ),
                "visual_comparisons" => $visual_comparisons,
                "market_results" => $market_results,
                "questions" => $questions_arr,
                "condition" => $this->ensure_string(
                    $data["condition"] ?? ($data["condition_notes"] ?? ""),
                ),
                "market" => $this->ensure_string(
                    $data["market"] ?? ($data["market_insights"] ?? ""),
                ),
                "signatures_marques_poincons" => $this->normalize_smp(
                    $data["signatures_marques_poincons"] ?? [],
                ),
            ]),
        );
        return $out;
    ...
    private function normalize_visual_comparisons($v)
    {
        ...
        foreach ($v as $item) {
            ...
            $idx = isset($item["match_index"])
                ? (int) $item["match_index"]
                : count($out);
            $verdict = trim((string) ($item["verdict"] ?? ""));
            $verdict_lower = strtolower(
                preg_replace("/[^a-z0-9àâäéèêëïîôùûü_]/", "", $verdict),
            );
            if (in_array($verdict_lower, ["identique"], true)) {
                $verdict = "identique";
            } elseif (
                in_array($verdict_lower, ["mememodele", "meme_modele"], true)
            ) {
                $verdict = "même_modèle";
    ...
    {
        $matches = $this->extract_lens_matches($lens_data);
        $out["visual_matches"] = $matches;
        ...
        if (
            ...
            foreach ($matches as $mat) {
                ...
                $txt = preg_replace(
                    "/\s+/",
                    " ",
                    trim(substr($mat[2], 0, 1500)),
                );
                if (strlen($txt) >= 50) {
                    $scraped_by_url[$u] =
                        $txt . (strlen($mat[2]) > 1500 ? "..." : "");
                }
            }
        }
        $domains_url_interdits = [
            "invaluable.com",
            "artnet.com",
            "drouot.com",
            "auction.fr",
        ...
        $url_verified = function ($url) use ($scraped_by_url) {
            ...
            if (isset($scraped_by_url[$u])) {
                return true;
            }
            $u_norm = rtrim($u, "/");
            return isset($scraped_by_url[$u_norm]) ||
                isset($scraped_by_url[$u_norm . "/"]);
        };
        $should_empty_url = function ($url) use (
            $scraped_by_url,
            $domains_url_interdits,
            $url_verified,
        ) {
        ...
        foreach ($matches as $i => $m) {
            ...
            $correspondances[] = [
                ...
                "source" => $m["source"] ?? "",
                "match_type" => $m["match_type"] ?? "visual",
                "verdict" => $v["verdict"],
                "details" => $v["details"],
                "notes" => $notes,
            ];
        ...
        if (
            !$has_usable_correspondance &&
            preg_match("/élevée|elevee|elevée/i", $out["reliability"] ?? "")
        ) {
        ...
        if (empty($out["market_results"]) && !empty(trim($scraped_content))) {
            $excerpt = preg_replace(
                "/\s+/",
                " ",
                trim(substr($scraped_content, 0, 2000)),
            );
            if (strlen($excerpt) >= 100) {
                $out["market_results"] = [
                    [
                        "title" => "Contenu web récupéré",
                        "url" => "",
                        "price" => "",
                        "source" => "",
                        "relevance" => "",

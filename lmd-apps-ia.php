...
foreach ($files_to_load as $file) {
    if (!lmd_safe_require($file)) {
        add_action('admin_notices', function() use ($file) {
            echo '<div class="error"><p>LMD Apps IA : fichier manquant — ' . esc_html($file) . '</p></div>';
        });
        return;
    }
}
...
function lmd_init_plugin() {
    try {
        load_plugin_textdomain('lmd-apps-ia', false, dirname(LMD_PLUGIN_BASENAME) . '/languages');
        LMD_Migration::run();
        ...
        add_action('lmd_monthly_consumption_report', 'lmd_send_monthly_consumption_report');
    } catch (Exception $e) {
        error_log('LMD init error: ' . $e->getMessage());
        add_action('admin_notices', function () use ($e) {
            if (current_user_can('manage_options')) {
                echo '<div class="notice notice-error"><p><strong>LMD Apps IA :</strong> Erreur d\'initialisation — ' . esc_html($e->getMessage()) . '</p></div>';
            }
        });
    } catch (Error $e) {
        ...
        add_action('admin_notices', function () use ($e) {
            if (current_user_can('manage_options')) {
                echo '<div class="notice notice-error"><p><strong>LMD Apps IA :</strong> Erreur fatale — ' . esc_html($e->getMessage()) . '</p></div>';
            }
        });
    }
}
...
add_action('plugins_loaded', function () {
    ...
    update_option('lmd_imgbb_enabled', (string) get_option('lmd_imgbb_key', '') !== '');
    update_option('lmd_imgbb_prefs_v1', 1);
}, 4);

add_filter('plugin_action_links_' . LMD_PLUGIN_BASENAME, function ($links) {
    $links[] = '<a href="' . esc_url(admin_url('admin.php?page=lmd-apps-ia')) . '">Vue d\'ensemble</a>';
    return $links;
});
...
function lmd_send_monthly_consumption_report() {
    ...
    if (!get_option('lmd_consumption_monthly_enabled') || !class_exists('LMD_Api_Usage')) {
        if (is_multisite()) restore_current_blog();
        return;
    }
    $email = get_option('lmd_consumption_monthly_email', '');
    $emails = array_filter(array_map('sanitize_email', array_map('trim', explode(',', $email))));
    if (empty($emails)) {
        return;
    }
    ...
    $lines = [];
    $lines[] = "Client;Site ID;Analyses;SerpAPI (unités);SerpAPI ($);Firecrawl (unités);Firecrawl ($);ImgBB (unités);ImgBB ($);Gemini (unités);Gemini ($);Total ($)";
    foreach ($clients as $c) {
        ...
        $lines[] = implode(';', array_map(function ($v) { return str_replace([';', "\r", "\n"], [' ', ' ', ' '], (string) $v); }, $row));
    }
    $lines[] = "TOTAL;;" . $agg['analyses_count'] . ";" . $agg['by_api']['serpapi']['units'] . ";" . $agg['by_api']['serpapi']['cost_usd'] . ";" . $agg['by_api']['firecrawl']['units'] . ";" . $agg['by_api']['firecrawl']['cost_usd'] . ";" . $agg['by_api']['imgbb']['units'] . ";" . $agg['by_api']['imgbb']['cost_usd'] . ";" . $agg['by_api']['gemini']['units'] . ";" . $agg['by_api']['gemini']['cost_usd'] . ";" . $agg['total_usd'];
    $csv = "\xEF\xBB\xBF" . implode("\r\n", $lines);

    $subject = sprintf('[LMD Apps IA] Consommation IA %s', $month);
    $body = "Rapport de consommation IA pour le mois de " . $month . ".\n\n" . count($clients) . " client(s), " . $agg['analyses_count'] . " analyse(s), " . number_format($agg['total_usd'], 4) . " $ total.\n\nLe fichier CSV est joint.";
    $filename = 'consommation-ia-' . $month . '.csv';
    $tmp = wp_upload_bits($filename, null, $csv);
    if (!empty($tmp['file'])) {
        foreach ($emails as $to) {
            wp_mail($to, $subject, $body, ['Content-Type: text/plain; charset=UTF-8'], [$tmp['file']]);
        }
        @unlink($tmp['file']);
    }
    if (is_multisite()) {
